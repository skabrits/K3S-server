...
sudo echo "* * * * * root if ping -c 1 master-01.local; then sudo nginx -c /etc/nginx/nginx.conf; sudo nginx -s reload; sudo echo \"*/5 * * * * root update_masters.sh\" > /etc/cron.d/watchmaster; sudo rm -f /etc/cron.d/connect; fi" > /etc/cron.d/connect
